...
function doubleAll(numbers: number[]): number[] {
  // declare a function that takes in the parameter numbers [], return a number []
  const doubled: number[] = []; // create a new array to push the numbers into called doubled
...
function getKeys(object: Record<string, unknown>): string[] {
  const keys: string[] = []; // assigned variable keys to an empty array string
...
function getValues(object: Record<string, unknown>): unknown[] {
  const values: unknown[] = []; // assinged a variable "values" to an empty array
  for (const prop in object) {
